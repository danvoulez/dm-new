...
export async function loadContracts(client: PgClient): Promise<Map<string, ProcessContract>> {
  const result = await client.query<{ process_id: string; title: string; status: string; contract: ProcessContract }>(
    "SELECT process_id,title,status,contract FROM public.process_contracts ORDER BY process_id",
  );
  return new Map(result.rows.map((row) => {
    const contract = { ...row.contract, process_id: row.process_id, title: row.title || row.contract.title, status: row.status || row.contract.status };
