...
init_performance2();
var CATALOG = {
  "no_matching_process_contract": ["Ficou registrado, mas n\xE3o h\xE1 um tipo de solicita\xE7\xE3o que case com isto.", "Escolher tipo", "user"],
  "process_not_active": ["Este tipo de solicita\xE7\xE3o est\xE1 desativado no momento.", "Avisar respons\xE1vel", "operator"],
  "incomplete": ["Para andar, falta: {campos}.", "Completar", "user"],
  "no_adapter_configured": ["Este tipo existe, mas ainda n\xE3o executa nada.", "Avisar respons\xE1vel", "operator"],
  "adapter_not_registered": ["A a\xE7\xE3o que este tipo pede ainda n\xE3o existe neste sistema.", "Avisar respons\xE1vel", "operator"],
  "dispatch_mismatch": ["A solicita\xE7\xE3o foi preparada com uma a\xE7\xE3o diferente da que a regra pede agora. Nada foi executado.", "Reenviar", "user"],
  "missing_required_grant": ["Para andar, precisa de autoriza\xE7\xE3o assinada.", "Pedir autoriza\xE7\xE3o", "user"],
  "grant_not_found": ["A autoriza\xE7\xE3o indicada n\xE3o existe.", "Pedir autoriza\xE7\xE3o", "user"],
  "grant_subject_mismatch": ["Esta autoriza\xE7\xE3o foi dada para outra pessoa.", "Pedir autoriza\xE7\xE3o", "user"],
  "grant_process_mismatch": ["Esta autoriza\xE7\xE3o n\xE3o vale para este tipo de solicita\xE7\xE3o.", "Pedir autoriza\xE7\xE3o", "user"],
  "grant_adapter_mismatch": ["Esta autoriza\xE7\xE3o n\xE3o cobre a a\xE7\xE3o que seria executada.", "Pedir autoriza\xE7\xE3o", "user"],
  "who_not_authorized": ["Voc\xEA n\xE3o est\xE1 na lista de quem pode fazer isto.", "Pedir acesso", "user"],
  "grant_not_active": ["Esta autoriza\xE7\xE3o n\xE3o est\xE1 ativa.", "Pedir autoriza\xE7\xE3o", "user"],
  "grant_revoked": ["Esta autoriza\xE7\xE3o foi cancelada.", "Pedir autoriza\xE7\xE3o", "user"],
  "grant_expired": ["A autoriza\xE7\xE3o venceu em {data}.", "Renovar", "user"],
  "budget_exhausted": ["O limite de uso desta autoriza\xE7\xE3o acabou.", "Pedir aumento", "user"],
  "missing_grant_expiry": ["A autoriza\xE7\xE3o n\xE3o tem prazo de validade. Por seguran\xE7a, n\xE3o vale.", "Avisar respons\xE1vel", "operator"],
  "missing_timeout": ["A autoriza\xE7\xE3o n\xE3o define tempo limite de execu\xE7\xE3o.", "Avisar respons\xE1vel", "operator"],
  "missing_sandbox_scope": ["A autoriza\xE7\xE3o n\xE3o define onde a a\xE7\xE3o pode mexer.", "Avisar respons\xE1vel", "operator"],
  "missing_network_policy": ["A autoriza\xE7\xE3o n\xE3o define o que a a\xE7\xE3o pode acessar na rede.", "Avisar respons\xE1vel", "operator"],
  "missing_authority": ["N\xE3o foi informado quem autoriza.", "Avisar respons\xE1vel", "operator"],
  "unregistered_authority": ["Quem concedeu n\xE3o est\xE1 registrado como autorizador.", "Avisar respons\xE1vel", "operator"],
  "grant_unsigned": ["A autoriza\xE7\xE3o ainda n\xE3o foi assinada com chave de seguran\xE7a.", "Assinar", "user"],
  "signoff_signer_mismatch": ["A assinatura \xE9 de outra pessoa, n\xE3o de quem concedeu a autoriza\xE7\xE3o.", "Assinar de novo", "user"],
  "signature_layer_unavailable": ["A verifica\xE7\xE3o por chave de seguran\xE7a n\xE3o est\xE1 dispon\xEDvel neste dispositivo.", "Ver como habilitar", "operator"],
  "evidence_obligation_unmet": ["A a\xE7\xE3o rodou, mas n\xE3o comprovou {campos}. Nada foi dado como conclu\xEDdo.", "Ver o que foi produzido", "operator"],
  "adapter_rejected": ["A a\xE7\xE3o foi recusada com seguran\xE7a: {motivo}.", "Ver detalhe", "operator"]
};
...
app.get("/v1/models", async (c) => {
  const res = await app.request("/api/models", {}, c.env);
...
app.get("/api/process-types", async (c) => {
  return c.json({
    ...
    types: [
      ...
      { process_id: "memory-register.v1", title: "Mem\xF3ria", requires: [], accepts: ["descricao"], required_slots: ["who", "did", "this"], adapter: "receipt", danger_tier: "L0", needs_approval: false, irreversible: false, runnable: true, readiness: "runnable", readiness_reason: "", evidence_must_include: [] },
...
app.get("/api/cases/:hash", async (c) => {
  ...
  void pgConn(c);
  return c.json({ hash, fingerprint: fingerprint(hash), found: false, valid: false, slots: {}, fields: {}, timeline: [{ step: "registered", label: "Registrado", when: (/* @__PURE__ */ new Date()).toISOString(), hash, fingerprint: fingerprint(hash) }], came_from: [], produced: [] }, 200);
});
...
app.get("/api/projections", async (c) => {
  try {
    ...
    const projections = rows.results.map((r) => ({ ...r, fingerprint: fingerprint(r.projection_hash), authoritative: false, rebuildable: true }));
    return c.json({ count: projections.length, note: "Resumos reconstru\xEDveis. N\xE3o s\xE3o a fonte.", projections });
  } catch {
    return c.json({ count: 0, note: "Resumos reconstru\xEDveis. N\xE3o s\xE3o a fonte.", projections: [] });
  }
});
app.post("/api/migrate", async (c) => {
  ...
  try {
    ...
    const client = new Client2({ connectionString: connStr, ssl: { rejectUnauthorized: false } });
...
app.post("/api/process-types", async (c) => {
  ...
  try {
    ...
    await c.env.PROJECTIONS.prepare("INSERT OR REPLACE INTO process_type_proposals (process_id, title, body, hash, created_at) VALUES (?,?,?,?,?)").bind(body.process_id, body.title, JSON.stringify(body), hash, (/* @__PURE__ */ new Date()).toISOString()).run();
    return c.json({ ok: true, process_id: body.process_id, hash, note: "Tipo registrado como proposta. Em produ\xE7\xE3o gera processes/*.v1.yml e recarrega catalog." });
  } catch (e) {
    return c.json({ ok: true, process_id: body.process_id, note: "Stub: tipo aceito (D1 indispon\xEDvel)." });
  }
});
...
app.post("/api/register", async (c) => {
  ...
  try {
    ...
    if (connStr) {
      ...
      try {
        await client.connect();
        const act = { ...body, id: hash, receipt_version: "logline.receipt.v0", hashes: { tuple_hash: hash } };
        await client.query(
  ...
  if (!inserted && insertError && insertError.includes("does not exist")) {
    return c.json({ registered: true, id: hash, fingerprint: fingerprint(hash), activated: false, waiting: { message: "Registrado (DB ainda n\xE3o migrado \u2014 stub).", action: "Ver caso" }, warning: insertError }, 200);
  }
...
var index_default = {
  fetch: app.fetch,
  async scheduled(controller, env2, _ctx) {
    try {
