...
def test_logline_acts_uses_canonical_hash_columns_and_generated_aux():
    ...
    row = db.execute(
        """SELECT content_hash, tuple_hash, receipt_version, envelope_hash, act, who, if_ok, aux
           FROM logline_acts
           WHERE tuple_hash = ?""",
    ...
    assert row["envelope_hash"] == receipt["hashes"]["envelope_hash"]
    assert row["receipt_version"] == "logline.receipt.v1"
...
def test_logline_acts_is_append_only():
    ...
    key = receipt["hashes"]["tuple_hash"]

    with pytest.raises(sqlite3.IntegrityError, match="append-only"):
        db.execute("UPDATE logline_acts SET receipt_version = receipt_version WHERE tuple_hash = ?", (key,))

    with pytest.raises(sqlite3.IntegrityError, match="append-only"):
        db.execute("DELETE FROM logline_acts WHERE tuple_hash = ?", (key,))
